...
- Tipo salvo pelo Java neste computador: `PKCS12`
...
4. Terminal mostrando `Servidor rodando!`.
5. Aviso de seguranca do navegador.
6. Pagina funcionando em `https://localhost:8443`.
7. Informacoes do certificado no navegador.

Observacao importante: o JDK 25.0.3 foi instalado e atende ao requisito de Java 11 ou superior. Se `javac` ainda nao for reconhecido no terminal, use o caminho completo `C:\Program Files\Java\jdk-25.0.3\bin\javac.exe` ou ajuste o PATH do Windows para apontar para a pasta `bin` do JDK.

## Roteiro curto para video de 3 a 5 minutos

### Parte 1 - Gerar o certificado (1 min) - JOAO

Joao mostra o terminal, roda ou explica o comando `keytool` e fala que ele cria um certificado digital autoassinado. Ele destaca o alias `meuservidor`, o algoritmo RSA, a chave de 2048 bits, a validade de 365 dias e o `CN=localhost`. Depois mostra que o arquivo `keystore.jks` foi criado na pasta do projeto.

### Parte 2 - Executar o servidor (1 min) - JOAO

Joao mostra os arquivos `ServidorHTTPS.java`, `index.html` e `keystore.jks`. Depois compila com `javac ServidorHTTPS.java` e executa com `java ServidorHTTPS`, mostrando no terminal a mensagem `Servidor rodando!` e o endereco `https://localhost:8443`.

### Parte 3 - Acessar no navegador (2 min) - BARBARA

Barbara acessa `https://localhost:8443`, mostra o aviso de seguranca e explica que ele aparece porque o certificado e autoassinado. Depois aceita o risco em `localhost`, mostra a pagina funcionando e clica no cadeado para mostrar as informacoes do certificado.

### Parte 4 - Explicar (1 min) - BARBARA

Barbara explica que HTTPS e importante porque criptografa a comunicacao entre navegador e servidor, ajudando a proteger dados contra interceptacao. Tambem pode comentar que o grupo aprendeu a gerar um certificado, carregar o `keystore.jks` no Java e testar uma pagina HTTPS local.
